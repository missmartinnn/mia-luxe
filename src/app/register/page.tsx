...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1b. If successful, immediately log them in securely using NextAuth
      ...
      if (signInResult?.error) {
        ...
        router.refresh(); // Sync the server state
  ...
  const handleGoogleSignIn = () => {
    // Note: Google sign-ins default to "customer" in the database.
    // If a Google user wants to sell, they can upgrade their account later!
    const callback = accountType === "seller" ? "/dashboard" : "/";
    ...
    <div className="min-h-screen bg-neutral-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-neutral-600">
          ...
          <Link href="/login" className="font-bold tracking-widest text-[10px] uppercase text-pink-500 hover:text-pink-600 transition-colors">
            Sign in here
          </Link>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-sm border border-neutral-200 sm:rounded-xs sm:px-10">
          ...
          {/* ACCOUNT TYPE TOGGLE */}
          <div className="flex bg-neutral-100 p-1 rounded-xs mb-6">
            <button
              type="button"
              onClick={() => setAccountType("customer")}
              className={`flex-1 py-2 text-xs font-bold tracking-widest uppercase rounded-xs transition-all ${
                accountType === "customer" ? "bg-white text-neutral-900 shadow-sm" : "text-neutral-500 hover:text-neutral-700"
              }`}
            >
              Shop
            </button>
            <button
              ...
              className={`flex-1 py-2 text-xs font-bold tracking-widest uppercase rounded-xs transition-all ${
                accountType === "seller" ? "bg-neutral-900 text-white shadow-sm" : "text-neutral-500 hover:text-neutral-700"
              }`}
            >
              Sell
            </button>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="pt-2">
              ...
              >
                {isLoading ? "Creating account..." : `Sign up to ${accountType}`}
              </button>
            </div>
          </form>
